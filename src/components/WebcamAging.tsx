...
export default function WebcamAging() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (video.readyState >= 2 && video.currentTime !== lastTime) {
        ...
        // Apply skin aging effect to main canvas
        if (showSkinAging) {
          ...
          for (let i = 0; i < d.length; i += 4) {
            ...
            d[i] = d[i] * (1 - blend) + (grey + 8) * blend;       // R: warm shift
            d[i + 1] = d[i + 1] * (1 - blend) + (grey - 2) * blend; // G: slight reduce
            d[i + 2] = d[i + 2] * (1 - blend) + (grey - 6) * blend; // B: reduce more
          }
  ...
  }, [cameraOn, modelReady, ageIntensity, showWrinkles, showHairGreying, showSkinAging]);
            ...
            {!cameraOn ? (
              ...
              >
                {loading ? (
                  ...
                  <><Loader2 className="w-4 h-4 animate-spin" /> Loading face model...</>
                ) : (
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-4 min-h-[400px] flex items-center justify-center overflow-hidden">
            {cameraOn ? (
              ...
              <div className="text-center space-y-3">
                ...
                <p className="text-xs text-slate-300">Requires camera permission. All processing happens locally.</p>
              </div>
            )}
          </div>
        </div>
...
function drawWrinkles(ctx: CanvasRenderingContext2D, lm: Landmark[], w: number, h: number, factor: number) {
  ...
  // Forehead wrinkles (landmarks: 10=forehead center, 67/297=forehead sides)
  ...
  // Crow's feet (around eyes: 33=left eye outer, 263=right eye outer)
  const crowsFeetPairs = [
    { outer: lm[33], dir: -1 },  // left eye
    { outer: lm[263], dir: 1 },  // right eye
  ];
  ...
  // Nasolabial folds (48=nose base left, 278=nose base right, to mouth corners)
  ...
  ctx.quadraticCurveTo(
    px(noseLeft, w) - 5,
    (py(noseLeft, h) + py(mouthLeft, h)) / 2,
    px(mouthLeft, w) - 3,
    py(mouthLeft, h) + 4
  );
  ctx.stroke();
  ...
  ctx.quadraticCurveTo(
    px(noseRight, w) + 5,
    (py(noseRight, h) + py(mouthRight, h)) / 2,
    px(mouthRight, w) + 3,
    py(mouthRight, h) + 4
  );
  ctx.stroke();

  // Under-eye lines (159=under left eye, 386=under right eye)
...
function drawHairGreying(ctx: CanvasRenderingContext2D, lm: Landmark[], w: number, h: number, factor: number) {
  // Get forehead top area and lighten/desaturate it
